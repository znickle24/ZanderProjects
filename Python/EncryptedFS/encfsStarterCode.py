...
class EncFS(Operations):
    ...
    @logged
    def decryptionHelper(self, path):
        """
        This is to help the getattr function to get the correct length of the
        file in order to not add a bunch of zero's when I Truncate
        """
        if path in self.memory:
            return self.memory[path]
        fi = open(self._full_path(path), "rb")
        salt = fi.read(16)
        kdf = PBKDF2HMAC(
    ...
    def getattr(self, path, fh=None):
        ...
        if not os.path.isdir(full_path):
            decrypted = self.decryptionHelper(path)
            dictionary['st_size'] = len(decrypted)
        return dictionary
...
'''
    ...
    def read(self, path, length, offset, fh):
        ...
        """
        file = self.memory[path]
        read_length = offset+length
        # if len(file) <= offset or len(file) <= read_length:
        #     return 0
        read = file[offset: read_length]
        return read
    ...
    def truncate(self, path, length, fh=None):
        ...
        if len(file) == length:
            return 0
        if len(file) > length:
            self.memory[path] = file[:length]
            return 0
        else :
            offset = length - len(file)
            zeroArray = [0] * offset
            file = file + zeroArray
            self.memory[path] = file
            return 0
    #skip
    '''
    @logged
