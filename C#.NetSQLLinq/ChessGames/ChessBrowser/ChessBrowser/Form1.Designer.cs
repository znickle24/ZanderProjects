...
namespace ChessBrowser
{
    partial class Form1
    {
        ...
        private void UploadGamesToDatabase(string PGNfilename)
        {
            ...
            List<ChessGame> games = ChessTools.PGNReader.ReadFromFile(PGNfilename);
            ...
            using (MySqlConnection conn = new MySqlConnection(connection))
            {
                try
                {
                    ...
                    foreach (ChessGame game in games)
                    {
                        ...
                        command.CommandText = ("insert ignore into chess_event VALUES (" + "\"" + game.GetEventName() + "\"" + "," + "\"" + game.GetSite() + "\"" + "," + "\"" + game.GetDate() + "\"" + ");");
                        command.ExecuteNonQuery();
                        command.CommandText = "insert ignore into chess_players VALUES (\"" + game.GetWhitePlayer() + "\");";
                        command.ExecuteNonQuery();
                        command.CommandText = "insert ignore into chess_players VALUES (\"" + game.GetBlackPlayer() + "\");";
                        command.ExecuteNonQuery();
                        command.CommandText = "insert into chess_game (white_player, event_name, game_result, moves, black_player) VALUES (\"" + game.GetWhitePlayer() + "\" , \"" + game.GetEventName() + "\" , \"" + game.GetResult() + "\" , \"" + game.GetMoves() + "\" , \"" + game.GetBlackPlayer() + "\");";
                        command.ExecuteNonQuery();
                        WorkStepCompleted();
                    }
          ...
          string winner, bool useDate, DateTime start, DateTime end, bool showMoves)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connection))
            {
                ...
                try
                {
                    ...
                    MySqlCommand command = conn.CreateCommand();
                    string preparedStatement = "select chess_event.event_name, site, date, white_player, black_player, game_result, moves from chess_game natural join chess_event where ";
                    bool isFirst = true;
                    if (white != "")
                    {   
                        preparedStatement += "white_player = @whiteVal ";
                        command.Parameters.AddWithValue("@whiteVal", white);
                        isFirst = false; 
                    }
                    if (black != "")
                    {
                        if (isFirst) {
                            preparedStatement += "black_player = @blackVal ";
                            command.Parameters.AddWithValue("@blackVal", black);
                            isFirst = false; 
                        } else
                        {
                            preparedStatement += " AND black_player = @blackVal "; 
                            command.Parameters.AddWithValue("@blackVal", black);
                        }
                    }
                    if (opening != "")
                    {
                        ...
                        {
                            opening = "1." + opening + "%";
                            preparedStatement += "moves like" + "@openingVal";
                            command.Parameters.AddWithValue("@openingVal", opening);
                            isFirst = false;
                        }
                        else
                        {
                            opening = "1." + opening + "%";
                            preparedStatement += " AND moves like" + "@openingVal";
                            command.Parameters.AddWithValue("@openingVal", opening);
                        }
                    }
                    if (winner != "")
                    {
                        ...
                        {
                            if (winner == "Black") {
                                preparedStatement += "game_result = \'b\'";
                                isFirst = false;
                            } else if (winner == "White")
                            {
                                preparedStatement += "game_result = \'w\'";
                                isFirst = false;
                            } else
                            {
                                preparedStatement += "game_result = \'d\'";
                                isFirst = false;
                            }
                        ...
                        else
                        {
                            if (winner == "Black")
                            {
                                preparedStatement += " AND game_result = \'b\'";
                                isFirst = false;
                            }
                            else if (winner == "White")
                            {
                                preparedStatement += " AND game_result = \'w\'";
                                isFirst = false;
                            }
                            else
                            {
                                preparedStatement += " AND game_result = \'d\'";
                                isFirst = false;
                            }
                        }
                    }
                    if (useDate)
                    {
                        if (isFirst)
                        {
                            preparedStatement += "date BETWEEN \'" + start.ToString("yyyy-M-dd") + "\' AND \'" + end.ToString("yyyy-M-dd") + "\'";
                            isFirst = false;
                        }
                        else
                        {
                            preparedStatement += " AND date BETWEEN \'" + start.ToString("yyyy-M-dd") + "\' AND \'" + end.ToString("yyyy-M-dd")+ "\'";
                        }
                    }
                    //if (showMoves)
                    //{
                    //    if (isFirst)
                    //    {
                    //        preparedStatement += "moves";
                    //        isFirst = false;
                    //    }
                    //    else
                    //    {
                    //        preparedStatement += "AND moves";
                    //    }
                    //}
                    Console.WriteLine(preparedStatement);
                    command.CommandText = preparedStatement;
                    command.ExecuteNonQuery();
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        ...
                        {
                            numRows += 1; 
                            //follow format online 
                            parsedResult += "Event: " + reader["event_name"] + "\r\n";
                            parsedResult += "Site: " + reader["site"] + "\r\n";
                            parsedResult += "Date: " + reader["date"] + "\r\n";
                            parsedResult += "White: " + reader["white_player"] + "\r\n";
                            parsedResult += "Black: " + reader["black_player"] + "\r\n";
                            parsedResult += "Result: " + reader["game_result"] + "\r\n";
                            if (showMoves)
                            {
                                //grab moves from database 
